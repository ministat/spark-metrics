...
function list_driver_janino_grows_beyond_64k_count() {
  local driverHost=$1
  ...
  local scriptName=find_grows_beyond_64k_count.sh
cat << EOF > $scriptName
ssh -o StrictHostKeyChecking=no $driverHost grep "grows\ beyond\ 64" $gcTrackDir/spark.*.log|wc -l
EOF
  scp $scriptName $DEV_MACHINE:/home/b_carmel
  ssh $DEV_MACHINE "chown b_carmel:b_carmel /home/b_carmel/$scriptName"
  local gcCount=$(ssh $DEV_MACHINE "su - b_carmel -c 'sh /home/b_carmel/$scriptName'")
...
ssh -o StrictHostKeyChecking=no $driverHost grep "Full\ GC" $gcTrackDir/stdout|wc -l
EOF
